...
Master::Master(const Options& options, EngineType type)
  ...
  if (type == EngineType::VLM) {
    ...
    spec_options.model_path(options_.model_path())
        ...
        .pld_enable_request_stats(options_.pld_enable_request_stats())
        .pld_stats_log_interval(options_.pld_stats_log_interval())
        .pld_candidate_count(options_.pld_candidate_count())
        .pld_prefer_recent_match(options_.pld_prefer_recent_match())
        .pld_min_ngram_size(options_.pld_min_ngram_size())
        .pld_max_ngram_size(options_.pld_max_ngram_size())
        .pld_enable_adaptive(options_.pld_enable_adaptive())
        .pld_adaptive_window_size(options_.pld_adaptive_window_size())
        .pld_adaptive_disable_steps(options_.pld_adaptive_disable_steps())
        .pld_adaptive_min_draft_hit_rate(
            options_.pld_adaptive_min_draft_hit_rate())
        .pld_adaptive_min_accept_rate(options_.pld_adaptive_min_accept_rate())
