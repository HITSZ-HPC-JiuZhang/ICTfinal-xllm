...
```
    ...
    - **Use `enum class`** instead of plain `enum` to provide type
            safety and prevent implicit conversions.
...
```
    ...
    - **Use `explicit`** on any constructor that can be invoked with a single
            argument.This includes multi -
    parameter constructors where all parameters except the first
        have default values.
...
```
    ...
    - **Use `override`** when overriding virtual functions in derived
            classes.Never repeat the `virtual` keyword on overrides.

```cpp
                // Good
                ModelOutput
                forward(torch::Tensor tokens, ...) override;
...
```
        ...
        - **File -
        local helper functions**(not part of the public API)
            must be prefixed with `_`.-
        **Non -
        public member functions** of a class must be prefixed with `_`
            .
...
```python def _validate_config(config
            ...
            ->bool : ...
                     ...
                     class ModelLoader : def load(self, path
                                                  : str)
            ->Model
...
```
        ...
        - ** Use `torch::` namespace** instead
        of `at::` or `c10::` wherever possible.Prefer the highest
                         - level PyTorch C++ API.

```cpp
                           // Good
                           torch::Tensor output = torch::zeros({batch_size,
                                                                hidden_dim});
...
```
        ...
        - **Always `reserve()` before filling a `std::vector`** when the size is
           known
    or can be estimated.
