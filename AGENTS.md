...
- Ascend heterogeneous hardware, especially `Ascend 910B`.
...
## Current Task Objective @README_ict_final.md
- **Goal**: Optimize the inference speed (specifically Output TPS) for the **Qwen3.5-9B** model.
- **Test Cases**:
  - Single concurrency: 64K input + 1K output
  - Single concurrency: 128K input + 1K output
- **Hardware Platform**: 2 * Ascend 910B1 (64G)
